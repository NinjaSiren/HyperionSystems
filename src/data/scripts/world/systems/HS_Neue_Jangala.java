...
public class HS_Neue_Jangala {
    ...
    public void generate(SectorAPI sector) {
        ...
        PlanetAPI venusia;
        if(isUS) {
            String pType = new PLANET_TYPES().US_LAVA;
            if(rand() <= 0.33) pType = new PLANET_TYPES().US_VOLCANIC;
            venusia = system.addPlanet("hs_planet_venusia", //Unique id for this planet (or null to have it be autogenerated)
            ...
            if(rand() <= 0.33) pType = new PLANET_TYPES().LAVA_MINOR;
        ...
        if(isUS) {   
            ...
            if(rand() <= 0.33) pType = new PLANET_TYPES().US_DESERT_B;
            else if(rand() > 0.33 && rand() <= 0.66) pType = new PLANET_TYPES().US_DESERT_C;
            ...
            if(rand() > 0.5) pType = new PLANET_TYPES().DESERT_B;
            mercurios = system.addPlanet(
                    "hs_planet_mercurios", //Unique id for this planet (or null to have it be autogenerated)
                    star, //What the planet orbits (orbit is always circular)
                    "Mercurios", //Name
        ...
        if(isUS) {
            ...
            if(rand() > 0.5) pType = new PLANET_TYPES().US_GAS_GIANT_B;
        ...
        if(isUS) {
            ...
            String pType = new PLANET_TYPES().LAVA;
            if(rand() > 0.5) pType = new PLANET_TYPES().LAVA_MINOR;
        ...
        if(isUS) {
            ...
            if(rand() <= 0.167) pType = new PLANET_TYPES().US_BARREN_B;
            else if(rand() > 0.167 && rand() <= 0.334) pType = new PLANET_TYPES().US_BARREN_C;
            else if(rand() > 0.334 && rand() <= 0.501) pType = new PLANET_TYPES().US_BARREN_D;
            else if(rand() > 0.501 && rand() <= 0.668) pType = new PLANET_TYPES().US_BARREN_E;
            else if(rand() > 0.668 && rand() <= 0.835) pType = new PLANET_TYPES().US_BARREN_F;
            ...
            if(rand() <= 0.33) pType = new PLANET_TYPES().BARREN_B;
            else if(rand() > 0.33 && rand() <= 0.66) pType = new PLANET_TYPES().BARREN_C;
            sedina = system.addPlanet(
                    "hs_planet_sedinia",
                    marutaina,
                    "Sedinia",
